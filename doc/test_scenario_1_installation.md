...
2. Přejděte do požadované složky: `cd C:\Projects`
3. Naklonujte repozitář:
   ```
   git clone https://github.com/RobiNZajicek/database-eshop.git
   cd database-eshop
   ```
4. **Očekávaný výsledek:** Projekt je stažen do složky `database-eshop`.

### 2. Import databázové struktury
1. Otevřete SQL Server Management Studio (SSMS).
2. Připojte se k databázovému serveru (např. `localhost` nebo školní server).
3. Vytvořte novou databázi: `CREATE DATABASE eshop_db;`
4. Otevřete a spusťte skripty v tomto pořadí:
   - `src/sql/01_create_tables.sql`
   - `src/sql/02_create_views.sql`
   - `src/sql/03_seed_data.sql`
5. **Očekávaný výsledek:** Databáze obsahuje 5 tabulek a 2 pohledy.
...
1. Otevřete nový terminál ve složce `e-shop/src/backend`.
2. Spusťte:
   ```
...
### 5. Spuštění backendu
1. Ve složce `e-shop/src/backend` spusťte:
   ```
   python app.py
   ```
...
1. Otevřete nový terminál ve složce `e-shop`.
2. Spusťte:
   ```
   npm install
   ```
3. **Očekávaný výsledek:** Závislosti jsou nainstalovány.
